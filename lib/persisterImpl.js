...
var CriteriaBuilder = require("./query/criteriaBuilder");
var UpdateDocumentBuilder = require("./query/updateDocumentBuilder");
var ReadContext = require("./mapping/readContext");
var CallbackUtil = require("./core/callbackUtil");
var PersisterImpl = (function () {
    ...
    PersisterImpl.prototype.findOneById = function (id, callback) {
        ...
        if (entity !== undefined) {
            return process.nextTick(function () { return callback(null, entity); });
        }
    ...
    PersisterImpl.prototype.findOne = function (criteria, callback) {
        var _this = this;
        this._collection.findOne(criteria, function (err, document) {
            if (err)
                return callback(err);
            var result = _this._loadOne(document);
    ...
    PersisterImpl.prototype._findAll = function (query, callback) {
        var _this = this;
        this._prepareFind(query).toArray(function (err, documents) {
            if (err)
                return callback(err);
            var result = _this._loadAll(documents);
            callback(result.error, result.value);
        });
    };
    PersisterImpl.prototype.executeQuery = function (query, callback) {
        ...
        switch (query.kind) {
            ...
            case 2 /* FindEach */:
                ...
                break;
            case 4 /* FindEachSeries */:
                this._findEachSeries(query, callback);
                break;
            case 32 /* FindOneAndRemove */:
            case 64 /* FindOneAndUpdate */:
    ...
    PersisterImpl.prototype._findEach = function (query, callback) {
        ...
        function error(err) {
            ...
            callback(err);
            callback = function () {
            }; // if called for error, make sure it can't be called again
        }
    };
    // TODO: optimize this function by using underlying cursor from core directly
    ...
    PersisterImpl.prototype._findOneAndModify = function (query, callback) {
        var _this = this;
        var options = {
            remove: query.kind == 32 /* FindOneAndRemove */,
            new: query.wantsUpdated
        };
        this._collection.findAndModify(query.criteria, query.sortValue, query.updateDocument, options, function (err, response) {
            if (err)
                return callback(err);
            ...
            if (entity) {
                if (options.remove) {
                    ...
                    _this._session.notifyRemoved(result.value);
                }
                    ...
                    _this._refreshFromDocument(entity, document, function (err) {
                        if (err)
                            return callback(err);
    ...
    PersisterImpl.prototype._remove = function (query, callback) {
        var options = {
            single: query.kind == 256 /* RemoveOne */
        };
    ...
    PersisterImpl.prototype._update = function (query, callback) {
        var options = {
            multi: query.kind == 512 /* UpdateAll */
        };
    ...
    PersisterImpl.prototype._distinct = function (query, callback) {
        ...
        this._collection.distinct(context.resolvedPath, query.criteria, undefined, function (err, results) {
            ...
            var readContext = new ReadContext(_this._session);
    ...
    PersisterImpl.prototype._loadAll = function (documents) {
    ...
    PersisterImpl.prototype._loadOne = function (document) {
        var entity;
        if (!document) {
            entity = null;
        }
...
var BulkOperationCommand = (function () {
    function BulkOperationCommand(collection, mapping) {
        this._mapping = mapping;
        this.collectionName = collection.collectionName, this.operation = collection.initializeUnorderedBulkOp(), this.inserted = this.updated = this.removed = 0;
    }
    ...
    BulkOperationCommand.prototype.execute = function (callback) {
        ...
        this.operation.execute(function (err, result) {
            ...
            if ((result.nInserted || 0) != _this.inserted) {
                return callback(new Error("Flush failed for collection '" + _this.collectionName + "'. Expected to insert " + _this.inserted + " documents but only inserted " + (result.nInserted || 0) + "."));
            }
            if ((result.nModified || 0) != _this.updated) {
                return callback(new Error("Flush failed for collection '" + _this.collectionName + "'. Expected to update " + _this.updated + " documents but only updated " + (result.nModified || 0) + "."));
            }
            if ((result.nRemoved || 0) != _this.removed) {
                return callback(new Error("Flush failed for collection '" + _this.collectionName + "'. Expected to remove " + _this.removed + " documents but only removed " + (result.nRemoved || 0) + "."));
            }
            callback();
        });
    };
        ...
        this._persister.findAll({ _id: { $in: ids } }, function (err, entities) {
            if (!err) {
                for (var i = 0, l = entities.length; i < l; i++) {
                    ...
                    var id = entity["_id"].toString(), callback = callbacks[id];
                    callback(null, entity);
